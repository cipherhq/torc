...
describe('platformSettings (customer-web)', () => {
  beforeEach(() => {
    invalidatePlatformSettingsCache();
  });
  ...
  describe('DEFAULT_PLATFORM_SETTINGS', () => {
    ...
    it('has fee-related defaults', () => {
      expect(DEFAULT_PLATFORM_SETTINGS.tax_rate).toBe(8);
      expect(DEFAULT_PLATFORM_SETTINGS.hazard_fee).toBe(15);
      expect(DEFAULT_PLATFORM_SETTINGS.scheduling_fee).toBe(5);
      expect(DEFAULT_PLATFORM_SETTINGS.service_fee_pct).toBe(10);
    });
    ...
    it('all numeric values are numbers', () => {
      const numericKeys: (keyof PlatformSettingsMap)[] = [
        'platformFee', 'maxJobRadius', 'providerTimeout',
        'urgentSlaHours', 'standardSlaHours',
        'chat_max_message_length', 'chat_messages_per_page',
        'chat_history_retention_days', 'chat_conversations_per_page',
        'chat_max_image_size_mb', 'tax_rate', 'hazard_fee',
        'scheduling_fee', 'service_fee_pct',
      ];
