import { Phone, PhoneOff, Mic, MicOff, Volume2, VolumeX, AlertCircle, X } from 'lucide-react';
      ...
      try {
        ...
        channel.on('broadcast', { event: 'call_signal' }, async (payload) => {
          ...
          if (signal.type === 'ice-candidate' && signal.candidate) {
            try { await pc.addIceCandidate(new RTCIceCandidate(signal.candidate)); } catch (e) { console.warn('ICE error:', e); }
          }
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
        <button onClick={toggleMute} style={{
          ...
          {isMuted ? <MicOff style={{ width: 24, height: 24, color: '#EF4444' }} /> : <Mic style={{ width: 24, height: 24, color: '#FFFFFF' }} />}
        </button>
        ...
        <button onClick={toggleSpeaker} style={{
          ...
          {isSpeaker ? <Volume2 style={{ width: 24, height: 24, color: '#FFFFFF' }} /> : <VolumeX style={{ width: 24, height: 24, color: '#EF4444' }} />}
        </button>
      </div>
