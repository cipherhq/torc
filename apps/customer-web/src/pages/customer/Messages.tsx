...
export function CustomerMessages() {
  ...
  async function loadJobs() {
    ...
    if (error) {
      ...
      const rows = ((data || []) as JobRow[]).filter((row) => !!row.provider_id);
      const providerIds = Array.from(new Set(rows.map((row) => row.provider_id).filter(Boolean))) as string[];
      ...
      if (providerIds.length > 0) {
        ...
        if (!profileError && profiles) {
          nameMap = profiles.reduce((acc: Record<string, string>, profile: any) => {
            ...
            acc[profile.id] = fullName || 'Provider';
            return acc;
          }, {});
        }
      }
      ...
      setJobs(
        rows.map((row) => ({
          ...row,
          provider_name: row.provider_id ? nameMap[row.provider_id] || 'Provider' : 'Provider',
        }))
      );
    }
    setLoading(false);
  ...
  function getPeer(job: JobRow) {
    ...
    const last = parts[1] || '';
    const peerInitials = `${first[0] || 'P'}${last[0] || ''}`.toUpperCase();
        ...
        {loading ? (
          ...
          <div className="rounded-2xl p-10 text-center" style={{ backgroundColor: cardBg, border: `1px solid ${cardBorder}` }}>
            ...
            <p className="text-sm" style={{ color: subColor }}>
              Once a provider is assigned to your request, you can chat here.
            </p>
          </div>
        ) : (
          jobs.map((job, index) => {
