...
import { MapPin, Star, Clock, Navigation, Search, ChevronLeft, ExternalLink } from 'lucide-react';
...
export function ProviderExplore() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#FFFFFF' }}>
      ...
      <div className="p-6 pb-3" style={{ paddingTop: 'calc(env(safe-area-inset-top, 16px) + 16px)' }}>
        <div className="flex items-center gap-3 mb-4">
          <button
            onClick={() => navigate('/provider/home')}
            className="torc-btn-secondary w-10 h-10 rounded-full flex items-center justify-center"
            style={{ backgroundColor: '#F3F4F6', padding: 0 }}
          >
            <ChevronLeft className="w-5 h-5" style={{ color: '#1A1F2E' }} />
          </button>
          <h1 className="text-2xl font-bold" style={{ color: '#1A1F2E' }}>Explore Shops</h1>
        </div>
      ...
      <div className="px-4 mb-3">
        <div className="rounded-2xl overflow-hidden" style={{ height: 220, border: '1px solid #E5E7EB' }}>
          {isLoaded && currentPos ? (
            <GoogleMap
      ...
      {/* Shop list */}
      <div ref={listRef} className="flex-1 overflow-auto px-4 space-y-3" style={{ paddingBottom: 'calc(100px + env(safe-area-inset-bottom, 20px))' }}>
        {loading ? (
          <div className="flex flex-col items-center py-12">
            <div className="w-10 h-10 border-3 border-[#008CE5] border-t-transparent rounded-full animate-spin mb-3" />
            <p className="text-sm" style={{ color: '#6B7280' }}>Finding nearby auto shops...</p>
          </div>
        ) : shops.length === 0 ? (
          <div className="text-center py-12">
            <MapPin className="w-12 h-12 mx-auto mb-3" style={{ color: '#6B7280' }} />
            <p className="font-medium" style={{ color: '#1A1F2E' }}>No shops found nearby</p>
            <p className="text-sm mt-1" style={{ color: '#6B7280' }}>Try adjusting your search</p>
          </div>
        ) : (
          shops.map((shop) => (
            <button
              ...
              style={{
                backgroundColor: selectedShop?.place_id === shop.place_id ? 'rgba(78,205,196,0.08)' : '#FFFFFF',
                border: `1px solid ${selectedShop?.place_id === shop.place_id ? '#008CE5' : '#E5E7EB'}`,
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold truncate" style={{ color: '#1A1F2E' }}>{shop.name}</p>
                <p className="text-sm truncate mt-0.5" style={{ color: '#6B7280' }}>{shop.vicinity}</p>
              ...
              <div
                ...
                style={{ backgroundColor: 'rgba(78,205,196,0.1)', touchAction: 'manipulation' }}
              >
