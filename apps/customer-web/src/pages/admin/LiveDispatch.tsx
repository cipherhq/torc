    ...
    async function loadActiveJobs() {
      try {
        ...
        const formattedJobs: Job[] = (data || []).map((job: any, index: number) => {
          const customerName = job.customer?.user?.full_name || 'Unknown';
          ...
          if (job.status === 'en_route') status = 'enroute';
          else if (job.status === 'in_progress') status = 'inprogress';
          else if (job.status === 'matched' || job.status === 'accepted') status = 'matched';
          else if (job.status === 'requested') status = 'pending';
          ...
          // Mock location coordinates (would need actual geocoding)
          const x = 30 + (index * 15) % 50;
          ...
          return {
            ...
            location: { 
              x, 
              y, 
              address: job.pickup_address || 'Location not set' 
            },
            status,
            priority: diffMinutes > 30 ? 'urgent' : 'normal',
            timestamp,
            eta: status === 'enroute' ? Math.floor(Math.random() * 15) + 5 : undefined,
          };
        });
            ...
            >
              ...
              {loading ? (
                <div className="p-8 text-center">
                  <p className="text-white/60">Loading active jobs...</p>
                </div>
              ) : jobs.length === 0 ? (
                <div className="p-8 text-center">
                  <p className="text-white/60">No active jobs</p>
                </div>
              ) : (
