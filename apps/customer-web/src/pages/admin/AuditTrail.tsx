      ...
      <div className="p-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-white/60">Track admin actions and entity changes</p>
          </div>
          <div className="px-4 py-2 rounded-2xl bg-white/5 border border-white/10 text-white/80 text-sm">
        ...
        <div className="flex items-center gap-3 mb-6">
          <button
            ...
            className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white/90 text-sm flex items-center gap-2 hover:bg-white/10"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
          ...
          <div className="glass-light rounded-[24px] p-6">
            ...
            <p className="text-white/60 text-sm">Entity Types</p>
            <p className="text-white text-3xl font-bold">{entityOptions.length}</p>
          </div>
          <div className="glass-light rounded-[24px] p-6">
            <Filter className="w-8 h-8 text-[#FFA500] mb-3" />
            <p className="text-white/60 text-sm">Bulk Actions (Filtered)</p>
            <p className="text-white text-3xl font-bold">{bulkCount}</p>
          </div>
        </div>
        ...
        <div className="glass-light rounded-[24px] p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <label className="flex items-center gap-2 px-3 py-3 rounded-[16px] bg-white/5 border border-white/10 text-white/90">
              <input
                type="checkbox"
                checked={bulkOnly}
        ...
        <div className="glass-light rounded-[24px] overflow-hidden">
          {loading ? (
            <div className="p-12 text-center">
              <p className="text-white/60">Loading audit entries...</p>
            </div>
          ) : loadError ? (
            <div className="p-12 text-center">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="border-b border-white/10">
                  <tr>
                    <th className="px-6 py-4 text-left text-white/60 text-sm font-semibold">Timestamp</th>
                    <th className="px-6 py-4 text-left text-white/60 text-sm font-semibold">Actor</th>
                    <th className="px-6 py-4 text-left text-white/60 text-sm font-semibold">Action</th>
                    <th className="px-6 py-4 text-left text-white/60 text-sm font-semibold">Entity</th>
                    <th className="px-6 py-4 text-left text-white/60 text-sm font-semibold">Details</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredLogs.map((log, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <p className="text-white">{formatEntity(log.entity_type)}</p>
                        <p className="text-white/50 text-xs">{log.entity_id}</p>
                      </td>
                      <td className="px-6 py-4">
                        <pre className="text-white/70 text-xs whitespace-pre-wrap break-words max-w-md">
