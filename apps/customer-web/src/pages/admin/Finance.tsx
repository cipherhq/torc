...
export function AdminFinance() {
  ...
  async function loadFinance() {
    try {
      ...
      const [
        ...
        supabase
          .from('jobs')
          .select('id, total_amount, tip, completed_at, created_at, status, payment_status')
          .eq('status', 'completed'),
  ...
  const metrics = useMemo(() => {
    ...
    const grossSales = jobs.reduce((sum, job) => sum + num(job.total_amount), 0);
  ...
  const monthly = useMemo(() => {
    ...
    jobs.forEach((job) => {
      ...
      const amount = num(job.total_amount);
  ...
  const paymentStatusRows: Array<{ key: JobPaymentStatus; label: string; color: string }> = [
    { key: 'paid', label: 'Paid', color: 'text-[#9FFFD8]' },
    { key: 'requires_action', label: 'Requires Action', color: 'text-yellow-300' },
    { key: 'unpaid', label: 'Unpaid', color: 'text-white/70' },
    { key: 'failed', label: 'Failed', color: 'text-red-300' },
    { key: 'refunded', label: 'Refunded', color: 'text-orange-300' },
  ];

  const payoutStatusRows: Array<{ key: PayoutStatus; label: string; color: string }> = [
    { key: 'paid', label: 'Paid Out', color: 'text-[#9FFFD8]' },
    { key: 'processing', label: 'Processing', color: 'text-[#A5D8FF]' },
    { key: 'pending', label: 'Pending Queue', color: 'text-yellow-300' },
    { key: 'failed', label: 'Failed', color: 'text-red-300' },
  ];
      ...
      <div className="p-8">
        <div className="mb-8">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h1 className="text-4xl font-bold text-white mb-2">Financial Hub</h1>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="rounded-xl bg-[#008CE5]/20 border border-[#008CE5]/30 text-[#9FFFD8] px-4 py-2 text-sm flex items-center gap-2 disabled:opacity-60"
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
              <motion.div className="glass-light rounded-[24px] p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <p className="text-white/60 text-sm">Gross Completed Sales</p>
                <p className="text-white text-3xl font-bold">{money(metrics.grossSales)}</p>
                <p className="text-white/60 text-xs mt-1">Service amount before fee deductions</p>
              </motion.div>
              <motion.div className="glass-light rounded-[24px] p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <CreditCard className="w-8 h-8 text-[#0070B8] mb-2" />
                <p className="text-white/60 text-sm">Captured Customer Payments</p>
                <p className="text-white text-3xl font-bold">{money(metrics.capturedCustomerPayments)}</p>
                <p className="text-white/60 text-xs mt-1">Paid charge volume from completed jobs</p>
              </motion.div>
              <motion.div className="glass-light rounded-[24px] p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <TrendingUp className="w-8 h-8 text-[#008CE5] mb-2" />
                <p className="text-white/60 text-sm">Platform Fees ({platformFeePercent.toFixed(1)}%)</p>
                <p className="text-white text-3xl font-bold">{money(metrics.expectedPlatformFees)}</p>
                <p className="text-white/60 text-xs mt-1">TORC fee retained from completed services</p>
              </motion.div>
              <motion.div className="glass-light rounded-[24px] p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <DollarSign className="w-8 h-8 text-[#9FFFD8] mb-2" />
                <p className="text-white/60 text-sm">Net Platform Revenue</p>
                <p className="text-white text-3xl font-bold">{money(metrics.netPlatformRevenue)}</p>
                <p className="text-white/60 text-xs mt-1">
                  Fee revenue minus approved refunds ({money(metrics.approvedRefunds)})
                </p>
              </motion.div>
              <motion.div className="glass-light rounded-[24px] p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <Wallet className="w-8 h-8 text-[#A5D8FF] mb-2" />
                <p className="text-white/60 text-sm">Provider Payout Owed</p>
                <p className="text-white text-3xl font-bold">{money(metrics.expectedProviderPayout)}</p>
                <p className="text-white/60 text-xs mt-1">Sales - TORC fee + tips ({money(metrics.totalTips)})</p>
              </motion.div>
              <motion.div className="glass-light rounded-[24px] p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <CheckCircle2 className="w-8 h-8 text-[#9FFFD8] mb-2" />
                <p className="text-white/60 text-sm">Paid Out To Providers</p>
                <p className="text-white text-3xl font-bold">{money(metrics.paidOut)}</p>
                <p className="text-white/60 text-xs mt-1">From payout ledger status = paid</p>
              </motion.div>
              <motion.div className="glass-light rounded-[24px] p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <Clock className="w-8 h-8 text-yellow-300 mb-2" />
                <p className="text-white/60 text-sm">Outstanding Payout Liability</p>
                <p className="text-white text-3xl font-bold">{money(metrics.providerOutstandingEstimate)}</p>
                <p className="text-white/60 text-xs mt-1">
              ...
              <motion.div className="glass-light rounded-[24px] p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <p className="text-white/60 text-sm">Cash Position Estimate</p>
                <p className="text-white text-3xl font-bold">{money(metrics.cashPositionEstimate)}</p>
                <p className="text-white/60 text-xs mt-1">Captured payments - refunds - paid payouts</p>
              </motion.div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              <div className="glass-light rounded-[24px] p-6">
                <p className="text-white/60 text-sm">Payment Success Rate</p>
                <p className="text-white text-3xl font-bold">{metrics.paymentSuccessRate.toFixed(1)}%</p>
                <p className="text-white/60 text-xs mt-1">Completed jobs with payment_status=paid</p>
              </div>
              <div className="glass-light rounded-[24px] p-6">
                <p className="text-white/60 text-sm">Payout Coverage</p>
                <p className="text-white text-3xl font-bold">{metrics.payoutCoveragePct.toFixed(1)}%</p>
                <p className="text-white/60 text-xs mt-1">Paid + processing vs provider amount owed</p>
              </div>
              <div className="glass-light rounded-[24px] p-6">
                <p className="text-white/60 text-sm">Platform Margin</p>
                <p className="text-white text-3xl font-bold">{metrics.marginPct.toFixed(2)}%</p>
                <p className="text-white/60 text-xs mt-1">Net platform revenue vs completed sales</p>
              </div>
            </div>

            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-8">
              <div className="glass-light rounded-[24px] p-6">
                <h2 className="text-white font-bold text-xl mb-4">Payment Status Breakdown</h2>
                <div className="space-y-3">
                  {paymentStatusRows.map((row) => (
                    <div key={row.key} className="rounded-2xl bg-white/5 border border-white/10 p-4">
                      ...
                      <p className="text-white text-xl font-bold mt-1">{money(metrics.paymentBuckets[row.key].amount)}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="glass-light rounded-[24px] p-6">
                <h2 className="text-white font-bold text-xl mb-4">Payout Status Breakdown</h2>
                <div className="space-y-3">
                  {payoutStatusRows.map((row) => (
                    <div key={row.key} className="rounded-2xl bg-white/5 border border-white/10 p-4">
                      ...
                      <p className="text-white text-xl font-bold mt-1">{money(metrics.payoutBuckets[row.key].amount)}</p>
                    </div>
                  ))}
                </div>
                {payouts.length === 0 && (
            ...
            <div className="glass-light rounded-[24px] p-6 mb-8">
              <h2 className="text-white font-bold text-xl mb-4">Monthly Financial Rollup</h2>
              {monthly.length === 0 ? (
            ...
            <div className="glass-light rounded-[24px] p-6">
              <div className="flex items-start gap-3">
                <AlertCircle className="w-5 h-5 text-[#A5D8FF] mt-0.5" />
                <div>
                  <h3 className="text-white font-semibold mb-2">Payout Formula Applied</h3>
