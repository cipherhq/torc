...
function unlock() {
  ...
  try {
    ...
    src.buffer = buf;
    src.connect(getOutputChain(ctx));
    src.start(0);
  } catch {
    // ignore
  }
...
export function playNotificationBell() {
  ...
  try {
    ...
    if (ctx.state === 'suspended') {
      ctx.resume().then(() => _playChime(ctx)).catch(() => {});
      return;
    }
...
export function playMessageSound() {
  ...
  try {
    ...
    if (ctx.state === 'suspended') {
      ctx.resume().then(() => _playDing(ctx)).catch(() => {});
      return;
    }
 ...
 * Pattern: two-tone "ring-ring" repeated every 2.5 seconds.
