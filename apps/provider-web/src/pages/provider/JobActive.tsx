...
function mapJobStatusToUi(status?: string): 'enroute' | 'arrived' | 'working' | 'photos' {
...
export function JobActive() {
  ...
  const { currentJob, fetchJob, updateJobStatus, cancelJob } = useJob();
  ...
  return (
    <div className="min-h-screen relative overflow-hidden pb-32">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20 p-6 glass border-t border-white/10">
        ...
        {/* Provider cancel action - allow provider to cancel while enroute/arrived/working */}
        {(status === 'enroute' || status === 'arrived' || status === 'working') && (
          <motion.button
            ...
            className="mt-3 w-full bg-white/5 rounded-[32px] py-4 font-semibold text-white text-sm"
          >
            Cancel Job
          </motion.button>
        )}
